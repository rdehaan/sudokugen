...
#     "color_trap",
#     """
#     derivable(Mode,unicolor_iff(C1,yes(V),C2,no(V))) :-
...
#             cell(C), in_group(C,G),
#             different_cells(C,C1), different_cells(C,C2).
#     derivable(Mode,unicolor_iff(C1,S1,C2,S2)) :-
#         derivable(Mode,unicolor_iff(C2,S2,C1,S1)).
#     derivable(Mode,unicolor_iff(C1,S1,C3,S3)) :-
#         use_technique(Mode,color_trap), deduction_mode(Mode),
#         derivable(Mode,unicolor_iff(C1,S1,C2,S2)),
#         derivable(Mode,unicolor_iff(C2,S2,C3,S3)),
#         different_cells(C1,C2), different_cells(C2,C3),
#         different_cells(C1,C3),
#         not certainly_not_erased(C1),
#         not certainly_not_erased(C2),
...
#         different_cells(C1,C2).
#
#     :- derivable(_,unicolor_iff(C1,no(V),C2,no(V))),
#         not solution(C1,V), solution(C2,V).
#     :- derivable(_,unicolor_iff(C1,no(V),C2,no(V))),
#         solution(C1,V), not solution(C2,V).
#     :- derivable(_,unicolor_iff(C1,yes(V),C2,yes(V))),
#         not solution(C1,V), solution(C2,V).
#     :- derivable(_,unicolor_iff(C1,yes(V),C2,yes(V))),
#         solution(C1,V), not solution(C2,V).
#     :- derivable(_,unicolor_iff(C1,no(V),C2,yes(V))),
#         not solution(C1,V), not solution(C2,V).
#     :- derivable(_,unicolor_iff(C1,no(V),C2,yes(V))),
#         solution(C1,V), solution(C2,V).
#     :- derivable(_,unicolor_iff(C1,yes(V),C2,no(V))),
#         not solution(C1,V), not solution(C2,V).
#     :- derivable(_,unicolor_iff(C1,yes(V),C2,no(V))),
#         solution(C1,V), solution(C2,V).
#     """
...
#         use_technique(Mode,color_wrap), deduction_mode(Mode),
#         value(V),
#         derivable(Mode,unicolor_iff(C1,yes(V),C2,yes(V))),
